...
const Contacts = () => {
  ...
  const handleMassDelete = async () => {
     ...
     try {
        ...
        fetchContacts();
     } catch (err: any) { toast.error("Error: " + err.message, { id: tid }); } finally { setIsDeleting(false); }
  };
  ...
  const handleMassGroupUpdate = async (e: React.FormEvent) => {
     ...
     try {
        ...
        setIsMassGroupOpen(false); setMassGroupName(""); fetchContacts();
     } catch (err: any) { toast.error(err.message, { id: tid }); } finally { setIsUpdatingGroup(false); }
  };
  ...
  const handleDeleteContact = async (contact: any) => {
    ...
    try {
      ...
      setContactToDelete(null); fetchContacts();
  ...
  const handleOpenChat = async (contact: any) => {
    ...
    if (!leadId) {
       ...
       try {
           ...
           setSelectedLead(newLead); setIsChatOpen(true); fetchContacts();
           return;
            ...
            {isManager && (
                ...
                <Select value={selectedStatusFilter} onValueChange={setSelectedStatusFilter}>
                   ...
                   <SelectContent className="bg-[#121214] border-[#222225] text-white rounded-xl">
                      ...
                      {financialStatuses.map(s => <SelectItem key={s.id} value={s.id} className="focus:bg-[#161618]">{s.id}</SelectItem>)}
                   </SelectContent>
                </Select>
                <Select value={debtFilter} onValueChange={setDebtFilter}>
                   ...
                   <SelectContent className="bg-[#121214] border-[#222225] text-white rounded-xl">
                      <SelectItem value="ALL" className="focus:bg-[#161618]">Cualquier Saldo</SelectItem>
                      <SelectItem value="CON_DEUDA" className="focus:bg-[#161618] text-amber-400">Con Deuda Activa</SelectItem>
                      <SelectItem value="SIN_DEUDA" className="focus:bg-[#161618] text-emerald-400">Sin Deuda</SelectItem>
                   </SelectContent>
                </Select>
                </>
            )}
