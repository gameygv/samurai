...
import { 
  ...
  GitBranch, User, RefreshCcw, Layers, History, RotateCcw, Send, Sparkles, Fingerprint, MessageSquare, AlertTriangle, Database, ImageIcon
} from 'lucide-react';
...
const AgentBrain = () => {
  ...
  const handleRefreshMaster = async () => {
    ...
    try {
      ...
      console.error("Error re-ensamblando kernel", error);
    } finally {
  ...
  const handleSaveSnapshot = async () => {
    ...
    try {
      ...
      toast.success('Snapshot guardado y aplicado.');
  ...
  const handleTune = async () => {
    ...
    try {
      ...
      toast.success("Cerebro actualizado en el Laboratorio.");
    } catch (error: any) {
      toast.error("Error en laboratorio: " + error.message);
          ...
          <div className="flex-1 min-h-0 relative">
            <TabsContent value="alma" className="absolute inset-0 m-0">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
                  <PromptEditor title="Alma de Samurai" icon={Bot} value={prompts['prompt_alma_samurai']} onChange={v => setPrompts({...prompts, prompt_alma_samurai: v})} placeholder="Misión y propósito..." />
               ...
               <Card className="bg-slate-900 border-slate-800 h-full flex flex-col overflow-hidden">
                  ...
                  <ScrollArea className="flex-1">
                     <Table>
                        ...
                        <TableBody>
                           {versions.length === 0 ? (
                             <TableRow><TableCell colSpan={3} className="text-center py-10 text-slate-500">No hay snapshots registrados.</TableCell></TableRow>
                           ) : versions.map(v => (
                              <TableRow key={v.id} className="border-slate-800 hover:bg-slate-800/30">
                                 ...
                                 <TableCell className="text-right pr-6"><Button variant="outline" size="sm" className="h-8 text-[10px]" onClick={() => { setPrompts(v.prompts_snapshot); toast.info("Snapshot cargado. No olvides GUARDAR."); }}><RotateCcw className="w-3 h-3 mr-1" /> RESTAURAR</Button></TableCell>
                              </TableRow>
                           ))}
                        </TableBody>
                     </Table>
                ...
                <Card className="bg-slate-950 border-slate-800 shadow-2xl flex-1 flex flex-col overflow-hidden border-t-4 border-t-blue-600">
                    <CardHeader className="border-b border-slate-800 bg-slate-900/50 py-4 flex flex-row items-center justify-between shrink-0 px-6">
                        ...
                        <Select value={simProfile} onValueChange={setSimProfile}>
                          <SelectTrigger className="w-40 bg-slate-950 border-slate-800 h-9 text-[10px]"><SelectValue /></SelectTrigger>
                          <SelectContent className="bg-slate-900 text-white border-slate-800">
                            <SelectItem value="NORMAL">Lead Normal</SelectItem>
                            <SelectItem value="SIN_DATOS">Sin Datos Previos</SelectItem>
                            <SelectItem value="HOSTIL">Lead Difícil</SelectItem>
                          </SelectContent>
                        </Select>
                    </CardHeader>
        ...
        <Dialog open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen}>
           <DialogContent className="bg-slate-900 border-slate-800 text-white max-w-sm shadow-2xl">
              <div className="p-6 space-y-4">
                ...
                <p className="text-xs text-slate-400">Guarda el estado actual del cerebro antes de salir a producción.</p>
                ...
                <Button onClick={handleSaveSnapshot} disabled={saving || !snapshotName} className="bg-indigo-600 w-full h-11 font-bold">
                    {saving ? <Loader2 className="animate-spin w-4 h-4" /> : 'Confirmar & Aplicar'}
                </Button>
              </div>
           </DialogContent>
        </Dialog>
