...
const AgentBrain = () => {
  ...
  const restoreMasterPrompts = async () => {
     setRestoring(true);
     const tid = toast.loading("Rescatando memoria maestra...");
     try {
        ...
        toast.success("¡Memoria Restaurada! Pulsa Guardar Snapshot.", { id: tid });
        fetchPrompts();
     } finally { setRestoring(false); }
  };

  const handleSaveSnapshot = async () => {
    if (Object.keys(prompts).length === 0) return toast.error("No hay prompts para guardar.");
    setSaving(true);
    try {
      ...
      await supabase.from('prompt_versions').insert({
        version_name: snapshotName || `Update ${new Date().toLocaleDateString()}`,
        prompts_snapshot: prompts,
        created_by_name: profile?.username || 'Admin'
      });
      toast.success('Snapshot aplicado al Kernel.');
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col h-[calc(100vh-140px)] gap-6 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0 px-1">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-xs uppercase tracking-tighter">Estación de Mando de Inteligencia Artificial</p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={v => setSearchParams({ tab: v })} className="flex-1 flex flex-col min-h-0 bg-slate-900/20 rounded-2xl border border-slate-800/50 p-1">
          <TabsList className="bg-slate-950/50 border border-slate-800 p-1 mb-2 flex-wrap h-auto w-full justify-start shrink-0">
             ...
             <TabsTrigger value="snapshots" className="gap-2 shrink-0 data-[state=active]:bg-indigo-600"><GitBranch className="w-4 h-4"/> 3. Snapshots</TabsTrigger>
             ...
             <TabsTrigger value="tuner" className="gap-2 shrink-0 data-[state=active]:from-indigo-600 data-[state=active]:to-purple-600"><Sparkles className="w-4 h-4 text-yellow-400"/> 6. Laboratorio IA</TabsTrigger>
          ...
          <div className="flex-1 min-h-0 relative">
            ...
            <TabsContent value="alma" className="absolute inset-0 m-0">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full">
                  ...
                  <Card className="bg-slate-900 border-slate-800 border-l-4 border-l-emerald-500 flex flex-col h-full overflow-hidden shadow-2xl">
                    <CardHeader className="shrink-0 py-3 border-b border-slate-800 bg-slate-950/20"><CardTitle className="text-white text-xs flex items-center gap-2 uppercase tracking-widest"><Layers className="w-4 h-4 text-emerald-400" /> Jerarquía Constitucional</CardTitle></CardHeader>
                    <ScrollArea className="flex-1 p-6">
                      <div className="space-y-4">
                          <KernelStep num={1} title="Alma & ADN Core" desc="Personalidad y misión suprema." color="text-indigo-400" icon={Bot}/>
                          <KernelStep num={2} title="Estrategia de Cierre" desc="Protocolo táctico de ventas." color="text-blue-400" icon={Target}/>
                          <KernelStep num={3} title="Media Manager" desc="Trigger visual por ciudad." color="text-yellow-500" icon={ImageIcon}/>
                          <KernelStep num={4} title="Verdad Maestra" desc="Datos reales de theelephantbowl.com." color="text-emerald-400" icon={Database}/>
            ...
            <TabsContent value="identidad" className="absolute inset-0 m-0">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                  <PromptEditor title="ADN Core (Personalidad)" icon={Fingerprint} value={prompts['prompt_adn_core']} onChange={v => setPrompts({...prompts, prompt_adn_core: v})} placeholder="Cargando tono..." />
                  <PromptEditor title="Estrategia de Cierre" icon={Target} value={prompts['prompt_estrategia_cierre']} onChange={v => setPrompts({...prompts, prompt_estrategia_cierre: v})} placeholder="Cargando táctica..." color="text-blue-400" />
                </div>
            </TabsContent>
            ...
            <TabsContent value="snapshots" className="absolute inset-0 m-0">
               <Card className="bg-slate-900 border-slate-800 h-full flex flex-col overflow-hidden shadow-2xl">
                  <CardHeader className="shrink-0 border-b border-slate-800 pb-3 flex flex-row items-center justify-between"><CardTitle className="text-white text-xs flex items-center gap-2 uppercase tracking-widest"><History className="w-5 h-5 text-indigo-400" /> Línea de Tiempo</CardTitle></CardHeader>
                  <ScrollArea className="flex-1">
                     <Table>
                        <TableHeader><TableRow className="border-slate-800 bg-slate-950/20"><TableHead className="pl-6 text-[10px]">Versión</TableHead><TableHead className="text-[10px]">Fecha</TableHead><TableHead className="text-right pr-6 text-[10px]">Acción</TableHead></TableRow></TableHeader>
                        <TableBody>
                           {versions.length === 0 ? (
                             <TableRow><TableCell colSpan={3} className="text-center py-20 text-slate-500 italic">No hay snapshots registrados en el sistema.</TableCell></TableRow>
                           ) : versions.map(v => (
                              <TableRow key={v.id} className="border-slate-800 hover:bg-slate-800/30">
                                 ...
                                 <TableCell className="text-right pr-6"><Button variant="outline" size="sm" className="h-7 text-[10px] border-indigo-500/30 text-indigo-400" onClick={() => { setPrompts(v.prompts_snapshot); toast.info("Snapshot cargado. No olvides GUARDAR."); }}><RotateCcw className="w-3 h-3 mr-1" /> RESTAURAR</Button></TableCell>
                              </TableRow>
                           ))}
                        </TableBody>
                     </Table>
            ...
            <TabsContent value="vision" className="absolute inset-0 m-0">
                <PromptEditor title="Ojo de Halcón (Instrucciones OCR)" icon={EyeIcon} value={prompts['prompt_vision_instrucciones']} onChange={v => setPrompts({...prompts, prompt_vision_instrucciones: v})} placeholder="Cargando visión..." color="text-red-400" />
            </TabsContent>
                    ...
                    <CardHeader className="border-b border-slate-800 bg-slate-900/50 py-3 flex flex-row items-center justify-between shrink-0 px-6">
                        ...
                        <Select value={simProfile} onValueChange={setSimProfile}>
                          ...
                          <SelectContent className="bg-slate-900 text-white border-slate-800">
                            <SelectItem value="NORMAL">Lead Normal</SelectItem><SelectItem value="SIN_DATOS">Sin Datos</SelectItem><SelectItem value="HOSTIL">Lead Hostil</SelectItem>
                          </SelectContent>
                        </Select>
                    </CardHeader>
                    <ScrollArea className="flex-1 p-6 bg-black/10">
                        <div className="space-y-6 max-w-4xl mx-auto">
                           {simMessages.length === 0 && <div className="py-24 text-center text-slate-700 italic">Consola lista. Envía un mensaje para validar el Kernel.</div>}
                           {simMessages.map((msg, i) => (
                              <div key={i} className={cn("flex flex-col gap-2", msg.role === 'user' ? 'items-end' : 'items-start')}>
                                 ...
                                 {msg.explanation && <div className="text-[9px] text-slate-500 italic ml-4 bg-slate-900/50 p-2.5 rounded-lg border border-slate-800/50 max-w-[80%]"><strong>Kernel:</strong> {msg.explanation.reasoning}</div>}
                              </div>
                           ))}
                           {simulating && <div className="text-slate-500 flex gap-2 items-center text-xs p-4 animate-pulse"><Loader2 className="w-3 h-3 animate-spin text-blue-500"/> Sam consultando el Kernel...</div>}
                           <div ref={simEndRef} />
                        </div>
                    </ScrollArea>
                        ...
                        <div className="space-y-6 max-w-4xl mx-auto">
                           {tunerMessages.length === 0 && (
                              <div className="text-center py-24 opacity-40">
                                 ...
                                 <p className="text-xs mt-2 italic">"Sam, cambia tu tono para que seas más empático con el dolor."</p>
                              </div>
                           )}
                           ...
                           {tuning && <div className="p-4 text-xs text-purple-400 animate-pulse font-mono flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin"/> RE-ESCRIBIENDO CÓDIGO CORE...</div>}
                           <div ref={tunerEndRef} />
                        </div>
                    </ScrollArea>
                    <div className="p-4 bg-slate-900 border-t border-slate-800 shrink-0">
                        <div className="flex gap-3 max-w-4xl mx-auto items-end">
                           <Textarea value={tunerInput} onChange={e => setTunerInput(e.target.value)} placeholder="Indica una corrección maestra..." className="bg-slate-950 border-slate-700 min-h-[50px] max-h-40 resize-none py-3 text-sm focus-visible:ring-purple-500 shadow-inner" onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleTune(); }}} />
                ...
                <Card className="bg-slate-900 border-slate-800 shadow-2xl relative flex-1 flex flex-col overflow-hidden">
                    <div className="absolute top-4 right-6 z-10">
                       <Button onClick={handleRefreshMaster} variant="outline" className="h-9 text-[10px] border-indigo-500/50 text-indigo-400 bg-slate-950 hover:bg-indigo-500/20 font-bold" disabled={loadingMaster}>
                          {loadingMaster ? <Loader2 className="w-3 h-3 animate-spin mr-2"/> : <RefreshCcw className="w-3 h-3 mr-2"/>} RE-ENSAMBLAR
                       </Button>
                    </div>
                    ...
                    <div className="flex-1 bg-black p-8 overflow-y-auto custom-scrollbar">
                       <pre className="text-[11px] text-slate-400 font-mono whitespace-pre-wrap leading-relaxed select-text">
                          {loadingMaster ? "Re-ensamblando constitución..." : masterPrompt || "Kernel vacío. Pulsa RE-ENSAMBLAR."}
                       </pre>
                    </div>
                </Card>
            </TabsContent>
