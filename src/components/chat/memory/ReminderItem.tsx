...
import { Bell, Trash2, Smartphone, User, Users } from 'lucide-react';
...
export const ReminderItem = ({ reminder, onUpdate, onRemove }: ReminderItemProps) => {
  const target = reminder.target || 'agent';
  ...
  <div className="p-3 bg-[#121214] border border-[#222225] rounded-xl space-y-3 relative group">
    ...
    <div className="flex gap-2 pr-8">
      <Input
        ...
        placeholder={isClient ? "Mensaje para el cliente..." : "Ej: Llamar para cierre..."}
        className="h-8 text-xs bg-[#0a0a0c] border-[#222225] text-slate-200 focus-visible:ring-blue-500 flex-1 rounded-lg"
      />
      <Button
         variant="outline"
         size="icon"
         className={cn("h-8 w-8 shrink-0 rounded-lg border", isClient ? "bg-indigo-900/20 border-indigo-500/30 text-indigo-400" : "bg-amber-900/20 border-amber-500/30 text-amber-400")}
         onClick={() => onUpdate(reminder.id, 'target', isClient ? 'agent' : 'client')}
         title={isClient ? "Enviar al Cliente" : "Recordatorio para Agente"}
      >
         {isClient ? <User className="w-3.5 h-3.5" /> : <Users className="w-3.5 h-3.5" />}
      </Button>
      <Button
         variant="outline"
         size="icon"
    ...
    {isClient && (
      <div className="text-[9px] text-indigo-400 font-bold uppercase tracking-widest px-1">
        Se enviará al cliente por WhatsApp
      </div>
    )}
