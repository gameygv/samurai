...
import { Loader2, DollarSign, CalendarDays, Wallet, CheckCircle2, Edit3, Save, Plus, Trash2, X } from 'lucide-react';
...
export const ManageCreditDialog = ({ open, onOpenChange, sale, onSuccess }: ManageCreditDialogProps) => {
  ...
  useEffect(() => {
     if (open && sale) {
        const sorted = (sale.installments || []).sort((a: any, b: any) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime());
        setEditableInstallments(sorted);
        setEditMode(false);
        setPaymentAmount('');
     }
  }, [open, sale]);
  ...
  const handleApplyPayment = async (e: React.FormEvent) => {
     ...
     const pendingInsts = editableInstallments.filter((i: any) => i.status === 'PENDING' || i.status === 'LATE');
  ...
  const handleUpdateEditable = (id: string, field: string, value: string) => {
      setEditableInstallments(prev => prev.map(i => i.id === id ? { ...i, [field]: value } : i));
  };

  const handleAddInstallment = () => {
      const newInst = {
          id: `temp-${Date.now()}`, sale_id: sale.id, installment_number: editableInstallments.length + 1,
          amount: 0, due_date: new Date().toISOString().split('T')[0], status: 'PENDING', isNew: true
      };
      setEditableInstallments([...editableInstallments, newInst]);
  };

  const handleRemoveInstallment = async (inst: any) => {
      if (inst.isNew) {
          setEditableInstallments(prev => prev.filter(i => i.id !== inst.id));
          return;
      }
      if (!confirm("¿Eliminar esta cuota permanentemente?")) return;
      setProcessing(true);
      try {
          await supabase.from('credit_installments').delete().eq('id', inst.id);
          setEditableInstallments(prev => prev.filter(i => i.id !== inst.id));
  ...
  const handleSaveRestructure = async () => {
      if (Math.abs(projectedSum - totalAmount) > 1) {
          if(!confirm(`ADVERTENCIA: La suma de cuotas ($${projectedSum}) no coincide con el total de la venta ($${totalAmount}). ¿Deseas guardar de todos modos?`)) return;
      }
      
      setProcessing(true);
      const tid = toast.loading("Guardando nuevo plan de pagos...");
      try {
          ...
          const { error } = await supabase.from('credit_installments').upsert(toUpsert);
          if (error) throw error;
          
          toast.success("Plan reestructurado correctamente.", { id: tid });
          setEditMode(false);
          onSuccess();
      } catch (err: any) {
      ...
      <DialogContent className="bg-[#0f0f11] border-[#222225] text-white max-w-4xl rounded-3xl p-0 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 flex flex-col md:flex-row min-h-0 bg-[#0a0a0c]">
           <div className="w-full md:w-80 bg-[#121214] border-r border-[#222225] p-6 flex flex-col gap-6 shrink-0 overflow-y-auto custom-scrollbar">
              ...
              {!editMode && sale.status !== 'PAID' && (
                 <form onSubmit={handleApplyPayment} className="space-y-4 pt-4 border-t border-[#222225]">
                    <div className="space-y-2">
                       <Label className="text-[10px] uppercase font-bold text-emerald-400 tracking-widest ml-1">Ingresar Abono Rápido</Label>
                       ...
                       <p className="text-[9px] text-slate-500 italic leading-relaxed">El sistema cubrirá automáticamente los vencimientos más antiguos y dividirá las cuotas si es necesario.</p>
                    </div>
              ...
              {editMode && (
                 <div className="pt-4 border-t border-[#222225] space-y-4">
                    <div className="bg-amber-950/20 border border-amber-900/50 p-4 rounded-xl text-center">
                       <p className="text-[10px] uppercase font-bold text-amber-500 tracking-widest">Suma Proyectada</p>
                       <p className={cn("text-xl font-mono font-bold mt-1", Math.abs(projectedSum - totalAmount) > 1 ? "text-red-400" : "text-emerald-400")}>
                          ${projectedSum.toLocaleString()}
                       </p>
                       <p className="text-[9px] text-amber-500/70 mt-2">Ajusta los montos a la derecha para que cuadre con el Valor Total.</p>
                    </div>
                    <Button type="button" onClick={handleAddInstallment} className="w-full bg-[#161618] hover:bg-[#222225] border border-[#333336] text-white font-bold uppercase tracking-widest h-11 rounded-xl text-[10px]">
                       <Plus className="w-4 h-4 mr-2"/> Añadir Cuota
                    </Button>
                 </div>
              )}
           </div>
           ...
           <div className="flex-1 flex flex-col min-h-0 bg-[#050505]">
              <div className="p-4 border-b border-[#222225] bg-[#161618] shrink-0 flex items-center justify-between">
                 ...
                 {sale.status !== 'PAID' && (
                    <Button 
                       ...
                       onClick={() => { if (editMode) { setEditableInstallments(sale.installments); setEditMode(false); } else { setEditMode(true); } }}
                       className={cn("h-8 text-[10px] uppercase font-bold tracking-widest border-[#333336]", editMode ? "bg-amber-600/20 text-amber-500 border-amber-500/30 hover:bg-amber-600/30" : "bg-[#0a0a0c] text-indigo-400 hover:text-indigo-300 hover:bg-[#121214]")}
                    >
                       {editMode ? <><X className="w-3.5 h-3.5 mr-1.5"/> Cancelar Edición</> : <><Edit3 className="w-3.5 h-3.5 mr-1.5"/> Reestructurar</>}
                    </Button>
                 )}
              </div>
                 ...
                 <Table>
                    <TableHeader>
                       <TableRow className="border-[#222225] hover:bg-transparent">
                          <TableHead className="text-[10px] uppercase text-slate-500 font-bold sticky top-0 bg-[#161618] z-10 pl-6">Cuota</TableHead>
                          ...
                          <TableHead className="text-[10px] uppercase text-slate-500 font-bold sticky top-0 bg-[#161618] z-10 text-right">Monto ($)</TableHead>
                    ...
                    <TableBody>
                       {downPayment > 0 && (
                          <TableRow className="border-[#222225] bg-emerald-900/5">
                             <TableCell className="pl-6"><span className="text-[10px] uppercase tracking-widest font-bold text-slate-300">Enganche</span></TableCell>
                             <TableCell className="text-[10px] text-slate-500 font-mono">-</TableCell>
                             <TableCell className="font-mono text-[11px] text-right font-bold text-emerald-400">${downPayment.toLocaleString()}</TableCell>
                             <TableCell className="text-right pr-6"><Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[9px] h-5 px-2">PAGADO</Badge></TableCell>
                          </TableRow>
                       )}
                       {editableInstallments.map((inst: any, idx: number) => {
                          ...
                          const canEdit = editMode && inst.status !== 'PAID';

                          return (
                          <TableRow key={inst.id} className={cn("border-[#222225] hover:bg-[#121214] transition-colors", inst.status === 'PAID' ? 'opacity-60' : '', inst.isNew ? 'bg-indigo-900/10' : '')}>
                             <TableCell className="pl-6 font-mono text-[10px] text-slate-400">#{idx + 1}</TableCell>
                             <TableCell>
                                {canEdit ? (
                                   <Input type="date" value={inst.due_date} onChange={e => handleUpdateEditable(inst.id, 'due_date', e.target.value)} className="h-8 text-[11px] bg-[#0a0a0c] border-[#333336] text-amber-400 w-[140px]" />
                                ) : (
                                   <span className={cn("font-mono text-[11px]", isLate ? "text-red-400 font-bold" : inst.status === 'PAID' ? "text-slate-500 line-through" : "text-amber-400")}>
                                      {new Date(inst.due_date).toLocaleDateString()}
                                   </span>
                                )}
                                {inst.paid_at && !canEdit && <span className="block text-[9px] text-emerald-500/70 font-mono mt-0.5">Pagado: {new Date(inst.paid_at).toLocaleDateString()}</span>}
                             </TableCell>
                             <TableCell className="text-right">
                                {canEdit ? (
                                   <div className="flex justify-end">
                                      <Input type="number" value={inst.amount} onChange={e => handleUpdateEditable(inst.id, 'amount', e.target.value)} className="h-8 text-[11px] font-mono bg-[#0a0a0c] border-[#333336] text-emerald-400 text-right w-[100px]" />
                                   </div>
                                ) : (
                                   <span className={cn("font-mono text-[11px] font-bold", inst.status === 'PAID' ? "text-slate-500" : "text-white")}>
                             ...
                             <TableCell className="text-right pr-6">
                                {canEdit ? (
                                   <Button variant="ghost" size="icon" onClick={() => handleRemoveInstallment(inst)} className="h-8 w-8 text-slate-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg"><Trash2 className="w-4 h-4"/></Button>
                                ) : (
              ...
              {editMode && (
                 <div className="p-4 bg-[#161618] border-t border-[#222225] shrink-0 animate-in slide-in-from-bottom-2">
                    <Button onClick={handleSaveRestructure} disabled={processing} className="w-full bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold uppercase tracking-widest text-[10px] h-11 rounded-xl shadow-lg">
                       {processing ? <Loader2 className="w-4 h-4 animate-spin mr-2"/> : <Save className="w-4 h-4 mr-2"/>} Confirmar Reestructuración
                    </Button>
                 </div>
              )}
           </div>
