...
serve(async (req) => {
  ...
  try {
    ...
    // Extraer Base64 (Deep Search)
    let base64 = findValue(body, 'base64');

    // Fetching externo si Evolution no incluyó el base64 directo
    if (!base64 && (isAudio || isImage) && evoUrl && evoKey) {
        try {
            ...
            console.error("[Webhook] Búsqueda externa de base64 falló:", e);
        }
    }
    ...
    if (isAudio && bytes) {
        try {
            ...
            if (whisperRes.ok) {
                ...
                console.log("[Webhook] Audio transcrito.");
            } else {
                messageText = `[TRANSCRIPCIÓN AUDIO]: (Fallo en la transcripción)`;
            }
        } catch (e) {
            console.error("Whisper error:", e);
            messageText = `[AUDIO NO RECONOCIDO]`;
        }
    }
    ...
    // OJO DE HALCÓN (VISIÓN Y VALIDACIÓN DE PAGOS)
    if (isImage && base64) {
    ...
    // PARSEO DE BANDERA DE CIERRE (OJO DE HALCÓN)
    if (rawAnswer.includes('---JSON---')) {
        const parts = rawAnswer.split('---JSON---');
        rawAnswer = parts[0].trim(); // Ocultamos el JSON para no enviarlo a WhatsApp
        try {
            const parsedJson = JSON.parse(parts[1].trim());
            if (parsedJson.payment_status) {
                paymentStatusToUpdate = parsedJson.payment_status;
            }
        } catch(e) {
            console.error("[Webhook] Error procesando JSON de Auditoría:", e);
        }
    }

    // SI HAY VALIDACIÓN DE PAGO, ACTUALIZAR EL LEAD Y GUARDAR COMPROBANTE
        ...
        if (bytes) {
            ...
            if (uploadData) {
                ...
                await supabaseClient.from('media_assets').insert({
                    title: `Comprobante Automático ${lead.nombre}`,
